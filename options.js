...
import { PendingChanges } from "./pending_changes.js";
...
advancedFonts.getEffectiveFontSize = (fontSizeKey, callback) => {
	advancedFonts.fontSizeSettings[fontSizeKey].getter({}, (details) => {
		const controllable = advancedFonts.isControllableLevel(
...
advancedFonts.getEffectiveFont = (script, genericFamily, callback) => {
	const pendingChanges = advancedFonts.pendingChanges;
	const details = { script: script, genericFamily: genericFamily };
 ...
 */
advancedFonts.refreshFontSize = (fontSizeSetting, size, controllable) => {
	fontSizeSetting.label.textContent = `Size: ${size}px`;
	const inputElement = fontSizeSetting.sliderContainer.querySelector(
		'input[type="range"]',
	);
	if (inputElement) {
...
advancedFonts.refresh = () => {
	const script = advancedFonts.getSelectedScript();
	let sample;
	...
	const sampleTexts = document.querySelectorAll(".sample-text-span");
	for (let i = 0; i < sampleTexts.length; i++)
		sampleTexts[i].textContent = sample;

	let setting;
	let callback;
	for (const genericFamily in advancedFonts.fontSettings) {
		setting = advancedFonts.fontSettings[genericFamily];
		callback = advancedFonts.refreshFont.bind(null, setting);
		advancedFonts.getEffectiveFont(script, genericFamily, callback);
	}

	for (const fontSizeKey in advancedFonts.fontSizeSettings) {
 ...
 */
advancedFonts.getSelectedScript = () => {
 ...
 */
advancedFonts.getSelectedFont = (fontList) =>
 ...
 *     fonts on the system.
 */
advancedFonts.populateFontLists = (fonts) => {
	for (const genericFamily in advancedFonts.fontSettings) {
		const list = advancedFonts.fontSettings[genericFamily].fontList;
 ...
 */
advancedFonts.handleFontListChange = (genericFamily, event) => {
	const script = advancedFonts.getSelectedScript();
	const font = advancedFonts.getSelectedFont(event.target);
 ...
 */
advancedFonts.setSelectedFont = (fontList, fontId) => {
	const script = advancedFonts.getSelectedScript();
	let i;
 ...
 */
advancedFonts.handleFontSizeSliderChange = (fontSizeKey, value) => {
 ...
 */
advancedFonts.isControllableLevel = (levelOfControl) =>
 ...
 */
advancedFonts.setFontSizeSlider = (inputElement, size, enabled) => {
...
advancedFonts.initFontSizeSetting = (fontSizeKey) => {
	const fontSizeSettings = advancedFonts.fontSizeSettings;
	const setting = fontSizeSettings[fontSizeKey];
	// const label = setting.label;
	const samples = setting.samples;
	...
	setting.getter({}, (details) => {
		const size = details.pixelSize.toString();
		const controllable = advancedFonts.isControllableLevel(
			details.levelOfControl,
		);
		advancedFonts.setFontSizeSlider(inputElement, size, controllable);
 ...
 * @param {string} script The script code.
 */
advancedFonts.clearSettingsForScript = (script) => {
 ...
 */
advancedFonts.closeDialog = (dialogId) => {
...
advancedFonts.applyImportedSettings = async (config) => {
 ...
 */
advancedFonts.exportSettings = async () => {
 ...
 */
advancedFonts.getCurrentSettings = async () => {
...
advancedFonts.initApplyAndResetButtons = () => {
	const applyButton = $("apply-settings");
	...
	$("reset-this-script-button").onclick = (_) => {
		const scriptList = $("scriptList");
		const scriptName = scriptList.options[scriptList.selectedIndex].text;
...
advancedFonts.systemFonts = {
	...
	mac: "system-ui, Lucida Grande, sans-serif",
...
advancedFonts.sortScripts = () => {
	let i;
	const scripts = advancedFonts.scripts;
	for (i = 0; i < scripts; ++i) {
		if (scripts[i].scriptCode === advancedFonts.COMMON_SCRIPT) break;
	}
	const defaultScript = scripts.splice(i, 1)[0];
...
advancedFonts.initScriptList = () => {
	...
	for (let i = 0; i < scripts.length; i++) {
		const script = document.createElement("option");
